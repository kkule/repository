...
!(async () => {
  ...
  console.log("活动入口： 首页--领京豆--抢京豆  每天执行一次，暂时没解决自动开团生成互助码的问题，需要手动开团，或者等待大佬修复");
  message = ''
      console.log(`\n*********************************************【开始获取互助码】******************************************\n`);
...
function help(groupCode, shareCode) {
  ...
  return new Promise(resolve => {
    $.get(taskUrl('signGroupHelp', `{\"activeType\":2,\"groupCode\":\"${groupCode}\",\"shareCode\":\"${shareCode}\",\"activeId\":\"114\",\"source\":\"guest\"}`, groupCode, shareCode), async (err, resp, data) => {
      try {
        if (err) {
...
function getcode() {
  return new Promise(async (resolve) => {
    let options = taskUrl("signBeanGroupStageIndex", `{"rnVersion":"3.9","fp":"-1","shshshfp":"-1","shshshfpa":"-1","referUrl":"-1","userAgent":"-1","jda":"-1","monitor_source":"bean_m_bean_index","monitor_refer":""}`)
            ...
            if (data && safeGet(data)) {
              ...
              if (data.code == 0) {
                if (data.data.shareCode && data.data.groupCode) {
                  ...
                  console.log(`未开团成功，请手动进入活动页面进行开团\n\n`);
                }
              } else {//助力过该好友
                // message += `助力错误：${JSON.stringify(data.data.helpResult.errMsg)}\n}`
...
function taskUrl(function_id, body) {
  return {
    url: `${JD_API_HOST}/?functionId=${function_id}&body=${encodeURIComponent(body)}&appid=ld&client=android&clientVersion=10.0.4&networkType=wifi&osVersion=10&uuid=1653661333662603-8323134603933683&openudid=1653661333662603-8323134603933683&eu=1653661333662603&fv=8323134603933683&jsonp=jsonp_1625224497986_65507`,
    headers: {
      "Accept": "*/*",
